...
items = [
    Item(weight=rng.randint(0, 2), value=rng.randint(1, 3))
    for _ in range(3)
]
...
x = [
    knapsack.new_binary_var(f"x_{i}")
...
knapsack.add_objective(sum(
    x_i * i.value for x_i,i in zip(x,items)
))
...
x = [
    colorable_subgraph.new_var(f"x_{i}")
    for i in range(n)
]
for u, v in G.edges:
    colorable_subgraph.add_constraint(
        x[u] != x[v]
    )
